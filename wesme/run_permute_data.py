...
if args.output_prefix is not None:
    ...
    cur_permuted_dir = os.path.dirname(output_file)
else:
...
# JD: create cur_permuted_dir if not exists
try:
    os.makedirs(cur_permuted_dir)
except OSError:
    if not os.path.isdir(cur_permuted_dir):
        raise
...
# JD added seed argument to permute_mut_graph, seed = fid
permuted_graphs = {}
permuted_graphs[cantype] = permute_mut_data.permute_mut_graph(mut_graphs[cantype], genes, range(nsamples), pnum, seed = fid)
