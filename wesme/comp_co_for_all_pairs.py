"""
compute WeSCO pvalue for all pairs
...
logging.debug("comp CO for %s, %s" %(cantype, mtype))
...
if args.output is not None:
    ...
    pv_file = results_dir + cantype + "_" + mtype + "_co_pvs_"+str(pth)+".txt"
...
for i in range(len(genes)):
    ...
    for j in range(i+1, len(genes)):
        ...
        ws_pv, ws_ex_cover_sizes_dic = ws_mut_ex.compute_me_co_pv_ws\
            (covers, ws_k_cover_dic, max_pair_num=10**5, me_co="co", ws_ex_cover_sizes_dic=ws_ex_cover_sizes_dic)
        if ws_pv > pth:  # if pvalue is not significant enough
            continue
        ws_pv_dic[gene_pairs] = ws_pv # WeSCO pvalue
        # compute pvalue using hypergeometric test
        h_pv_dic[gene_pairs] = misc_mut_ex.compute_co_pv_hypergeom(alt_list_dic[gene1], alt_list_dic[gene2], nsamples)
