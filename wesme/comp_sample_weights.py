...
# Directory
data_dir = data_dir+can+"/"

# INPUT_FILES
if args.input is not None:
    smut_file = args.input
else:
    smut_file = data_dir + "_".join([can, mtype]) + "_list.txt"
...
# JD: create output dir if not exists
try:
    os.makedirs(os.path.dirname(smut_freq_file))
except OSError:
    if not os.path.isdir(os.path.dirname(smut_freq_file)):
        raise
