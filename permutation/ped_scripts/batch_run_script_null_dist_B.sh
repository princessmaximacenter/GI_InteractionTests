...
{
  echo "usage:
  batch_run_script_null_dist_B.sh -p project -m vmem -c cantype -t hrs -s startseed -o stopseed -l logdir
...
do
    ...
    proj_name=${project//[\/]/_}
    logdirstep=${logdir}/step4B_null_dist/${d}_${proj_name}_${ct}
    mkdir -p ${logdirstep}

    echo "working on": "${ct}"

    sbatch --array=${startseed}-${stopseed}%100 --time=${hr}:00:00 --mem=${vmem} \
    -e ${logdirstep}/"J4B_"${ct}_\%A.e\%a \
    -o ${logdirstep}/"J4B_"${ct}_\%A.o\%a \
    -J "J4B_"${proj_name}"_"${ct} \
    ./ped_scripts/run_script_null_dist_B.sh ${ct} ${project}
done
