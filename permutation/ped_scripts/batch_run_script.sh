...
{
  echo "usage:
  batch_run_script.sh -p project -m vmem -c cantype -t hrs -s startseed -o stopseed -n nperm -l logdir
...
do
    ...
    proj_name=${project//[\/]/_}
    logdirstep=${logdir}/step2_null_dist/${d}_${proj_name}_${ct}/
    mkdir -p ${logdirstep}

    sbatch --array=${startseed}-${stopseed}%200 --time=${hr}:00:00 --mem=${vmem} \
    -e ${logdirstep}/"J2_"${ct}_\%A.e\%a \
    -o ${logdirstep}/"J2_"${ct}_\%A.o\%a \
    -J "J2_"${proj_name}"_"${ct} \
    ./ped_scripts/run_script.sh ${ct} ${project} ${nperm}
